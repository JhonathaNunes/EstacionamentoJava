...
public class FrmUsuario extends JInternalFrame {
	...
	public void criarTblUsers(){
		...
		tabbedPane.addTab("Usuários", null, scrollPane, null);
		...
		btnSalvar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (usrDAO.editUsuario(usr) == false){
					JOptionPane.showMessageDialog(null, "Erro ao modificar o usuário", "Erro", JOptionPane.ERROR_MESSAGE);
				}else{
					JOptionPane.showMessageDialog(null, "Usuário modificado com sucesso", "Sucesso!!", JOptionPane.INFORMATION_MESSAGE);
					criarTblUsers();
					tabbedPane.setSelectedComponent(scrollPane);
				}
	...
	public void deletarUsuario(){
		...
		obterInfUsuario(user);
		int resp;
		resp = JOptionPane.showConfirmDialog(null, "Deseja mesmo deletar o usuário " + user.getNome() + "?", "Confirmação",
				JOptionPane.YES_NO_OPTION);
		if (resp == 0) {
			if (usrDAO.delUsuario(user) == true) {
				JOptionPane.showMessageDialog(null, "Usuário deletado com sucesso!!", "Usuário Deletado!!",
						JOptionPane.INFORMATION_MESSAGE);
				criarTblUsers();
			} else {
				JOptionPane.showMessageDialog(null, "Erro na tentativa de deletar usuário!!",
						"Usuário não deletado", JOptionPane.ERROR_MESSAGE);
			}
		}else {
			JOptionPane.showMessageDialog(null, "Usuário não deletado!!", "Usuário Mantido!!",
					JOptionPane.INFORMATION_MESSAGE);
		}
